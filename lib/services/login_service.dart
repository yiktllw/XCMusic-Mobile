...
class LoginService {
  ...
  Future<String?> createQrImg(String key) async {
    try {
      ...
      final result = await ApiManager().api.loginQrCreate(
        key: key,
        qrimg: "true",
        timestamp: timestamp,
      );
  ...
  Future<Map<String, dynamic>?> checkQrStatus(String key) async {
    try {
      ...
      final result = await ApiManager().api.loginQrCheck(
        key: key,
        timestamp: timestamp,
      );
  ...
  /// 登录成功时保存 cookie
  Future<void> _saveCookieOnLoginSuccess(
    Map<String, dynamic> loginResult,
  ) async {
    try {
      AppLogger.config('开始保存登录信息...');
      AppLogger.config(
        'GlobalConfig状态: ${_globalConfig.isInitialized ? "已初始化" : "未初始化"}',
      );
      ...
      if (profile != null) {
        final userInfo = {
          ...
          'playlistBeSubscribedCount':
              profile['playlistBeSubscribedCount'] ?? 0,
