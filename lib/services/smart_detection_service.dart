...
class SmartDetectionService extends ChangeNotifier {
  static final SmartDetectionService _instance = SmartDetectionService._internal();
  ...
  Future<bool> _testSensorsAvailability() async {
    try {
      ...
      AppLogger.info('传感器总体可用性: $sensorsAvailable (加速度: $accelerometerWorking, 陀螺仪: $gyroscopeWorking)');
  ...
  Future<void> startMonitoring({
    ...
    if (!_hasPermission || !_sensorsAvailable) {
      AppLogger.warning('传感器不可用，无法启动智能监听: 权限=$_hasPermission, 传感器=$_sensorsAvailable');
      return;
    }
    ...
    try {
      ...
      AppLogger.info('智能检测监听已启动 - 传感器状态: 权限=$_hasPermission, 可用=$_sensorsAvailable, 详情=${statusInfo}');
    } catch (e) {
  ...
  void _analyzeMotionData(Timer timer) {
    ...
    try {
      ...
      if ((result.isSleeping || result.recommendedAction == RecommendedAction.extendToMinimum) && 
        ...
        AppLogger.info('触发智能调整: ${result.recommendedAction}, 下次最早触发时间: ${DateTime.now().add(Duration(minutes: _actionTriggerIntervalMinutes))}');
      }
  ...
  bool _shouldTriggerAction() {
    ...
    final shouldTrigger = timeSinceLastAction.inMinutes >= _actionTriggerIntervalMinutes;
    
    if (!shouldTrigger) {
      final remainingMinutes = _actionTriggerIntervalMinutes - timeSinceLastAction.inMinutes;
  ...
  SleepDetectionResult _generateSleepAnalysis({
    ...
    // 运动强度评分 (越低越可能睡眠)
    if (motionIntensity < 0.1) confidence += 30;
    else if (motionIntensity < 0.3) confidence += 20;
    else if (motionIntensity < 0.5) confidence += 10;
    
    // 运动变化评分 (越低越可能睡眠)
    if (motionVariability < 0.1) confidence += 25;
    else if (motionVariability < 0.3) confidence += 15;
    else if (motionVariability < 0.5) confidence += 5;
    ...
    return SleepDetectionResult(
      ...
      analysisDetails: '运动强度: ${motionIntensity.toStringAsFixed(2)}, '
